...
# 依赖: Process.ps1, Ui.ps1, Profile.ps1（由入口脚本 dot-source 加载）
...
function Install-Ccline {
    ...
    try {
        ...
        try {
            ...
            $patchResult = Invoke-ExternalCommand -Command "ccline" -Arguments @("--patch", $claudeCliArg) -TimeoutSeconds 30 -SuppressOutput
            if ($patchResult.Success) {
                $patchApplied = $true
                Write-UiSuccess "✓ CCometixLine patch 已应用"
            } else {
                Write-UiWarning "⚠ CCometixLine patch 未完成，可手动执行: ccline --patch"
                Write-UiDim "  错误信息: $($patchResult.Error)" -Level Debug
                Write-UiDim "  可手动执行: ccline --patch `"$claudeCliPath`"" -Level Detail
            }
        } catch {
            Write-UiWarning "⚠ CCometixLine patch 未完成，可手动执行: ccline --patch"
            Write-UiDim "  错误详情: $($_.Exception.Message)" -Level Debug
            if ($claudeCliPath) {
                Write-UiDim "  可手动执行: ccline --patch `"$claudeCliPath`"" -Level Detail
            }
        }
        ...
        Write-UiDanger "✗ $($stepResult.ErrorMessage)"
        return $stepResult
    }
}
