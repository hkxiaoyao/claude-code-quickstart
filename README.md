...
- **共享契约**：`installer/contracts/` 统一 StepId、Provider、MCP、ClaudeConfig 与模板语义
...
# 引导（PS 5.1+）
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser -Force
.\installer\windows\Bootstrap-ClaudeEnv.ps1
...
Windows 使用 `installer/windows/Manage-ClaudeEnv.ps1` 或远程 `manage.ps1`，macOS 使用 `installer/macos/Manage-ClaudeEnv.zsh` 或远程 `manage.sh`。两端都提供统一管理入口：
