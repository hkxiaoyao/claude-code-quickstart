...
> 生成时间：2026-03-06 | 覆盖率：92% (24/26 文件)
...
│   ├── build/                    # 构建工具目录
│   │   ├── Build-SingleFile.ps1  # 单文件打包构建脚本（3 个产物）
│   │   └── dist/                 # 构建产物输出（gitignored，由 CI 自动构建）
│   ├── core/                     # 9 个基础功能库（Ui/Process/Profile/Admin/Net/Registry/Bootstrap/McpManager/Provider）
│   └── steps/                    # 13 个安装步骤模块（语义化命名，含 Update 函数）
└── test-syntax.ps1               # PS7 全量语法校验工具
```

```mermaid
graph TD
    A[Bootstrap-ClaudeEnv.ps1<br/>PS 5.1] -->|安装 PS7 后引导| I[Install-ClaudeEnv.ps1<br/>PS 7+ 安装]
    I --> C[core/]
    I --> D[steps/]
    M[Manage-ClaudeEnv.ps1<br/>PS 7+ 管理] --> C
    M --> D
    C --> C1[Ui.ps1] & C2[Process.ps1] & C3[Profile.ps1]
    C --> C4[Admin.ps1] & C5[Net.ps1] & C6[Registry.ps1] & C7[Bootstrap.ps1]
    C --> C8[McpManager.ps1] & C9[Provider.ps1]
    D --> D1[NodeFnm~ApiKey<br/>基础环境]
    D --> D2[Ccline~CcgWorkflow<br/>进阶扩展]
    D --> D3[CodexCli/GeminiCli/OpenSpec<br/>可选工具]
```
...
| installer/core/ | [installer/core/CLAUDE.md](installer/core/CLAUDE.md) | 9 个核心基础库（含 Registry + McpManager + Provider） |
| installer/steps/ | [installer/steps/CLAUDE.md](installer/steps/CLAUDE.md) | 13 个安装步骤模块（含 Update 函数） |
...
| 约束 | 内容 |
|------|------|
| **HC-12** | ApiKey 管 API 连接：`env.ANTHROPIC_AUTH_TOKEN` + `env.ANTHROPIC_BASE_URL` + `modelMapping` + `providers/` Profile 文件；供应商管理通过 `Manage → 供应商管理` 完成（CRUD + 切换）；ClaudeConfig 管常用配置：语言、权限、超时、归因等（仅补缺失，不覆盖），不写入 `model`（用户自行选择）；供应商支持 智谱GLM / MiniMax / Kimi / 自定义 |
